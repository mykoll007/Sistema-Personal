...
/* ============================================================
   PERSONAL LOGADO (nome + foto topbar)
============================================================ */
async function carregarPersonalLogado() {
  try {
    ...
    if (nomeEl) nomeEl.textContent = personal.nome;
    if (fotoEl) fotoEl.src = fotoFinal;

    return personal;
  } catch (err) {
    console.error("Erro carregarPersonalLogado:", err);
    return null;
  }
}

/* ============================================================
   MENU PERFIL (User Information / Nav Item)
============================================================ */
function iniciarMenuPerfil() {
  const btnPerfil = document.getElementById("btnPerfil");
  const menuPerfil = document.getElementById("menuPerfil");

  // Se a página não tiver esses elementos, não quebra
  ...
  document.getElementById("btnLogoutMenu")?.addEventListener("click", (e) => {
    e.preventDefault();
    if (window.$) $("#logoutModal").modal("show");
    else logout();
  });
...
function abrirModalFeedbacks(alunoId) {
  ...
  const tituloEl = document.getElementById("modalFeedbacksTitulo");
  const subtituloEl = document.getElementById("modalFeedbacksSubtitulo");
  const resumoEl = document.getElementById("modalFeedbacksResumo");
  const listaEl = document.getElementById("modalFeedbacksLista");

  if (!tituloEl || !subtituloEl || !resumoEl || !listaEl) {
    console.error("Modal não encontrado no HTML (ids do modal).");
    return;
  }

  // título/subtitulo
  tituloEl.textContent = `Feedbacks de ${alunoSelecionado.aluno_nome}`;
  subtituloEl.textContent = alunoSelecionado.aluno_email ? alunoSelecionado.aluno_email : "";
  ...
  if (!alunoSelecionado.feedbacks.length) {
    listaEl.innerHTML = `<div class="feedback-empty">Este aluno ainda não enviou feedbacks.</div>`;
  } else {
    listaEl.innerHTML = alunoSelecionado.feedbacks
      .map((f) => {
  ...
  if (!window.$) {
    console.error("jQuery não carregou — Bootstrap modal precisa do jQuery aqui.");
    return;
  }
